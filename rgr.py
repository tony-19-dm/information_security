...
class FiatShamirVisualizer:
    ...
    def show_protocol_explanation(self):
        """Показать объяснение протокола"""
        print("\n" + "="*60)
        print("ОБЪЯСНЕНИЕ ПРОТОКОЛА ФИАТА-ШАМИРА")
        print("="*60)
        
        print("\n📚 ОСНОВНЫЕ ПАРАМЕТРЫ:")
        print("  N = p * q - модуль (p и q - большие простые числа)")
        print("  s - секретный ключ пользователя (gcd(s, N) = 1)")
        print("  v = s² mod N - открытый ключ пользователя")
        
        print("\n🔄 ПРОТОКОЛ (один раунд):")
        print("  1. Пользователь выбирает случайное r, вычисляет x = r² mod N")
        print("  2. Сервер выбирает случайный бит e (0 или 1)")
        print("  3. Пользователь вычисляет y = r * s^e mod N")
        print("  4. Сервер проверяет: y² ≡ x * v^e mod N")
        
        print("\n🔐 СВОЙСТВА ПРОТОКОЛА:")
        print("  • Нулевое разглашение: сервер не узнает секретный ключ s")
        print("  • Доказуемость: вероятность обмана 1/2 за раунд")
        print("  • После t раундов вероятность обмана: 1/2^t")
        
        print("\n🎯 ПРИМЕР (t=20 раундов):")
        print("  Вероятность обмана: 1/2^20 ≈ 0.00000095")
        print("  То есть ~1 шанс из 1,000,000")
    ...
    def interactive_mode(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                self.show_protocol_explanation()
